...
router.post(
    ...
    async ctx => {
        ...
        if (profile.length > 0) {
            const post = await Post.findById(id);
            const isLike = post.likes.filter(like => {
                return like.user.toString() === ctx.state.user.id
            }).length > 0
            if (isLike) {
                ctx.status = 400;
                ctx.body = { alreadyliked: "用户已经赞过" };
                return;
            }
...
router.post(
    ...
    async ctx => {
        ...
        if (profile.length > 0) {
            const post = await Post.findById(id);
            const isLike =
                post.likes.filter(like => {return like.user.toString() === ctx.state.user.id})
                    .length === 0;
            if (isLike) {
                ctx.status = 400;
