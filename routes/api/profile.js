...
/**
 * @route GET api/profile/user?user_id=123
 * @gesc 通过user_id获取个人信息接口地址
 * @access 接口是公开的
 */
...
router.post('/experience', passport.authenticate('jwt', { session: false }), async ctx => {
    ...
    if (profile.length > 0) {
        ...
        if (profileUpdate.ok == 1) {
            const proflie = await Profile.find({ user: ctx.state.user.id }).populate("user", ['name', 'avatar']);
            if (profile) {
                ctx.status = 200;
                ctx.body = proflie;
            }
...
router.delete('/experience', passport.authenticate('jwt', { session: false }), async ctx => {
    ...
    if (profile[0].experience.length > 0) {
        const removeIndex = profile[0].experience.map(item => {
            item.id.indexOf(exp_id)
        })
